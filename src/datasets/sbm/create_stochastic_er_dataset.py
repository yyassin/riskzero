"""
Create stochastic Erdős–Rényi bipartite matching datasets.

Graph structure
---------------
An (n_left + n_right) × n_right bipartite graph is built as follows:

1. A G(n_left, n_right, m_edges) random bipartite graph is sampled: m_edges are
   chosen uniformly at random from all n_left × n_right possible edges.
2. n_right dummy left nodes are appended, each connected to exactly one distinct
   right node via a type-2 (zero-weight) edge.  This guarantees every right node
   has at least one incident edge, and effectively guarantees a skipping option
   for each right node.
...
def get_cvar_bm(n_left, n_nodes, edges, weight_samples, cvar_alpha):
    ...
    # shortfall of scenario i below v (t_i = max(0, v - W_i(x))) - we will subtract the expected shortfall from v in the objective to get CVaR.  See, e.g.:
...
def create_graph(n_left, n_right, m_edges, seed):
    """Sample a stochastic Erdős–Rényi bipartite graph.
    ...
    m_edges : Number of random edges between original left and right nodes.
    ...
    # Sample m_edges unique random edges between original left and right nodes.
    flat_indices = rng.choice(n_left * n_right, size=m_edges, replace=False)
    orig_left = flat_indices // n_right  # in [0, n_left)
    right_nodes = flat_indices % n_right + n_left_total  # in [n_left_total, n_left_total+n_right)
    random_edges = np.column_stack((orig_left, right_nodes))
...
def create_stochastic_er_gnm_dataset(
    num_instances,
    n_left,
    n_right,
    m_edges,
    ...
    """Generate a dataset of stochastic bipartite matching instances.
    ...
    n_left           : Number of original left nodes.
    n_right          : Number of right nodes.
    m_edges          : Number of random edges per graph.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate stochastic Erdős–Rényi bipartite matching datasets."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--m-edges", type=int, default=360, help="Number of random edges per graph."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="datasets/stochastic_bm/instances_60_30_360",
...
if __name__ == "__main__":
    ...
    #       --num-instances 256 --save-dir datasets/stochastic_bm/instances_60_30_180
    #
    #   # CVaR MILP solver:
    #   python3 -m src.datasets.sbm.create_stochastic_er_dataset \
    #       --n-left 60 --n-right 30 --m-edges 180 \
    #       --num-instances 256 --save-dir datasets/stochastic_bm/instances_60_30_180_cvar \
    #       --solver cvar_milp --cvar-alpha 0.25 --n-weight-samples 512

    args = parse_args()
    create_stochastic_er_gnm_dataset(
