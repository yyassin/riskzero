"""
Create stochastic Barabasi-Albert maximum independent set datasets.

Graph structure
---------------
A G(n_nodes, m_edges) Barabasi-Albert random graph is sampled with
preferential attachment.  For each non-type-2 node a paired "dummy" type-2
neighbor is added, guaranteeing at least one zero-risk node in every
neighborhood.
...
Solver options
--------------
Two formulations are available for computing the "risk-averse" IS label:
...
RISK_MAPPING = np.array([1, 100, 50])
...
def get_cvar_mis(n_nodes, edge_set, weight_samples, cvar_alpha):
    ...
    # Constraints: each node or one neighbor must be in the IS
    for i in range(n_nodes):
        neighbors = [j for j in range(n_nodes) if (i, j) in edge_set or (j, i) in edge_set]
...
def create_graph(n_nodes, m_edges, seed=0):
    """Sample a stochastic Barabasi-Albert MIS graph.

    Parameters
    ----------
    n_nodes  : Number of base nodes (before dummy nodes are added).
    m_edges  : Total edges for the BA graph; must be divisible by n_nodes.
    ...
    node_types : Integer array of node types, one per node.
    """
    assert m_edges % n_nodes == 0, "m_edges must be divisible by n_nodes for BA graph"
    G = nx.barabasi_albert_graph(n=n_nodes, m=m_edges // n_nodes, seed=seed)
    ...
    # Copy over all original BA edges.
    for u, v in G.edges:
        new_G.add_edge(u, v)
...
def create_stochastic_ba_dataset(
    ...
    """Generate *num_instances* BA-graph MIS instances and write them to disk.

    Parameters
    ----------
    n_nodes       : Number of base nodes per instance.
    m_edges       : Edges for BA graph; must be divisible by n_nodes.
...
if __name__ == "__main__":
    ...
    #       --n-nodes 60 --m-edges 240 \
    #       --num-instances 1024 --save-dir datasets/stochastic_mis/instances_ba_60_240
    #
    #   # CVaR MILP solver:
    #   python3 -m src.datasets.smis.create_stochastic_ba_dataset \
    #       --n-nodes 60 --m-edges 240 \
    #       --num-instances 1024 --save-dir datasets/stochastic_mis/instances_ba_60_240_cvar \
    #       --solver cvar_milp --cvar-alpha 0.25

    parser = argparse.ArgumentParser(description="Generate stochastic BA MIS dataset.")
    parser.add_argument("--n-nodes", type=int, default=60)
    parser.add_argument("--m-edges", type=int, default=240)
    parser.add_argument("--num-instances", type=int, default=1024)
    parser.add_argument(
        "--save-dir", type=str, default="./datasets/stochastic_mis/instances_ba_60_240"
    )
