# TQL training loop
#
# Implements the full TQL training pipeline:
...
#
# TQL trains two heads jointly on a shared parameter set:
#   QRNetwork  — (future) quantile action-values conditioned on a single observation
#   TQLNetwork — (trajectory-level) quantile action-values conditioned on an observation history
# The TQL head is used for action selection; the QR head provides
# bootstrapped value targets for the TQL head, combined with MC sampled of the accumulated return.
...
from src.baselines.tql.network import create_qr_networks
from src.baselines.tql.util import (
    Transition,
    get_value_targets,
...
def run_experiment(args):
    """Run a full TQL experiment and return logged metrics.
    ...
    _qr_model, _tql_model, _init_model = create_qr_networks(
    ...
    def evaluate(rng_key: jnp.ndarray, params: optax.Params):
        ...
        def loop_fn(
            ...
            tql_q_out = _tql_model.apply(
                params,
                eval_history_state.obs[:, 1:],  # Use history up to this step
            )
            greedy_action = jnp.argmax(tql_q_out.q_values, axis=-1)
    ...
    def selfplay(
        ...
        Actions are selected greedily by the TQL head with probability
        ``1 - epsilon`` and uniformly at random otherwise.  Environments are
        ...
        def step_fn(
            ...
            greedy_action = jnp.argmax(tql_q_out.q_values, axis=-1)  # (b, num_nodes)
            random_action = jax.random.randint(
                key2, shape=greedy_action.shape, minval=0, maxval=args.num_actions
            )
            epsilon = calc_eps(
                step_num,
                args.epsilon_start,
    ...
    def learning_step(rng_key, params, target_params, opt_state, buffer_state):
        ...
        Jointly optimises the QR head (quantile-Huber loss against bootstrapped
        targets selected by the TQL head) and the TQL head (quantile-Huber loss
        against targets combining history-accumulated returns with QR bootstraps).
        ...
        obs_history = learn_batch.obs_history[:, 0]
        act_history = learn_batch.act_history[:, 0]

        target_qr, target_tql = get_value_targets(
            learn_batch, _qr_model, _tql_model, target_params, args
        ...
        def _loss_fn(
            ...
            qr_out = _qr_model.apply(
                params,
                obs_history[:, -2],  # Second to last observation in the history is s
            ...
            tql_out = _tql_model.apply(
                params,
                obs_history[:, :-1],  # Use history up to s
            )  # (b, num_quantiles, num_actions)
            chosen_action_q_dist = jnp.take_along_axis(
                tql_out.q_dist,
                # The last action taken is -1 (to get to s')
            ...
            tql_losses = jax.vmap(quantile_huber_loss, in_axes=(0, None, 0, None, None))(
                chosen_action_q_dist,  # (b, num_quantiles)
                tau_hats,  # (num_quantiles, )
                target_tql,  # (b, num_quantiles)
                args.huber_param,
                True,  # stop gradients for target
            )
            chex.assert_shape(tql_losses, (args.buffer_batch_size,))
            tql_loss = jnp.mean(tql_losses)

            return tql_loss + qr_loss, {
                "tql_loss": tql_loss,
                "qr_loss": qr_loss,
            }
    ...
    def train_loop_body(carry, iteration):
        ...
        jax.debug.print(
            "Iter {i} / {max_num_iters}, Loss: {loss:.4f}, QR Loss: {qr_loss:.4f}, TQL Loss: {tql_loss:.4f} (LR: {lr:.6f}, EPS: {eps:.4f}), Train Avg Return: {avg_return:.2f}, Avg Length: {avg_length:.2f}",
            i=iteration,
            max_num_iters=args.max_num_iters,
            loss=jnp.mean(loss),
            qr_loss=jnp.mean(losses["qr_loss"]),
            tql_loss=jnp.mean(losses["tql_loss"]),
            lr=current_lr,
            eps=current_epsilon,
...
if __name__ == "__main__":
    ...
    def save_logs(logs):
        ...
        """
        log_file_path = "./logs/tql/logger.log"
