# Graph QR-DQN training loop (node-only variant)
#
# Implements the full QR-DQN training pipeline for graph-structured
# environments (stochastic max-independent-set).
# Differs from the edge-featured variant in that:
#   - Observations are graph dicts (node features, senders, receivers, aux
#     scalars) with no edge features.
#   - SAGEConv layers are used instead of edge-aware GraphConv.
# Differs from non-graph training in:
...
#   - Evaluation computes Monte-Carlo CVaR via node-type simulation.
#
...
from src.baselines.graph.qrdqn.network import create_graph_qr_network
from src.baselines.graph.qrdqn.util import (
    Transition,
    calc_eps,
...
def run_experiment(args):
    ...
    def evaluate(rng_key: jnp.ndarray, params: optax.Params):
        ...
        def eval_one_batch(batch_idx, carry):
            ...
            max_steps = 100  # Arbitrary cap to prevent infinite loops in case of bugs
    ...
    def eval_fn(subkey, iteration, params):
        ...
        jax.debug.print(
            "Iter {i} / {max_num_iters}, Eval Reward: {r}, Eval risk: {r_risk} ({r_opt_risk}), Opt Expected: {r_opt_expected}",
            i=iteration,
...
if __name__ == "__main__":
    ...
    def save_logs(logs):
        ...
        """
        log_file_path = "./logs/graph_qrdqn/smis/logger.log"
