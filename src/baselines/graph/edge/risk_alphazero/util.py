# Risk AlphaZero (graph/edge) utilities
#
# Shared helpers used by the graph-edge Risk AlphaZero training loop.
#
# Contents
# --------
...
def get_train_targets(batch, target_params, prediction_apply, args):
    ...
    _, value_dist, _, _ = prediction_apply.apply(
        ...
        batch.obs["edge_features"][-1],
        batch.obs["senders"][-1],
        batch.obs["receivers"][-1],
        batch.obs["aux"][-1],
    )  # (b, num_quantiles)
    values = value_dist  # (b, num_quantiles)
