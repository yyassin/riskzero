# Graph Sampled-TQL training loop (edge-featured variant)
#
# Implements the full Sampled-TQL training pipeline for graph-structured
# environments (stochastic bipartite matching). Extends the edge-featured
...
# samples drawn from both heads, over the trajectory-level return distribution.
#
...
from src.baselines.graph.edge.sampled_tql.network import create_graph_networks
from src.baselines.graph.edge.sampled_tql.util import (
    Transition,
    calc_eps,
...
class Config(BaseModel):
    seeds: list[int] = []
    seed: int = 23
    env_name: str = "stochastic-bipartite-matching"
...
def run_experiment(args):
    """Run a full Graph edge Sampled-TQL experiment and return logged metrics.
    ...
    def evaluate(rng_key: jnp.ndarray, params: optax.Params):
        ...
        def eval_one_batch(batch_idx, carry):
            ...
            def loop_fn(tup):
                ...
                q_values = jax.vmap(distortion_fn, in_axes=(0, None))(samples, args.alpha_cvar)
...
if __name__ == "__main__":
    ...
    def save_logs(logs):
        ...
        """
        log_file_path = "./logs/sampled_tql/sbm/logger.log"
