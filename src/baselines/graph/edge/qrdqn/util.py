# Graph QR-DQN utilities (edge-featured variant)
#
# Shared helpers used by the graph edge QR-DQN training loop.
#
# Contents
# --------
# Epsilon schedule:  calc_eps
# Data structures:   Transition
# Setup helpers:     init_model_and_optim_edge, make_buffer
...
def init_model_and_optim_edge(env, model_init_fn, args):
    """Initialise model parameters and an Adam optimiser for edge-featured graphs.

    Constructs a dummy observation (including edge features) to trace the model,
    then builds an optax optimiser with optional linear LR decay.
