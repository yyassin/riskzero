# Graph Sampled-TQL neural networks (node-only variant)
#
# Implements two network heads over SAGEConv graph convolutions:
#
# GraphQNetwork (action-value head)
#   Node features → SAGEConv ×4 with residual LayerNorm
...
#   Same SAGEConv backbone but outputs a single scalar quantile distribution
...
#   nodes, acting as virtual global pooling nodes.
#
# Factory
...
def create_graph_networks(args):
    def init_model_fn(
        ...
        qr_out = jax.vmap(qr_net)(nodes, senders, receivers, aux)
        reward_history_out = jax.vmap(reward_history_net)(nodes, senders, receivers, aux)
