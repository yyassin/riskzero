# Graph Sampled-TQL training loop (node-only variant)
#
# Implements the full Sampled-TQL training pipeline for graph-structured
# environments (stochastic max-independent-set). Extends the node-only
# QR-DQN with a second GraphVNetwork head (reward-history model) whose
# distributional predictions are combined with the QR model's future-return
# estimates at action-selection time. CVaR is computed over Monte-Carlo
# samples drawn from both heads, over the trajectory-level distribution of returns.
#
...
from src.baselines.graph.sampled_tql.network import create_graph_networks
from src.baselines.graph.sampled_tql.util import (
    Transition,
    calc_eps,
...
def run_experiment(args):
    """Run a full Graph Sampled-TQL experiment and return logged metrics.
    ...
    _init_model, _qr_model, _reward_history_model = create_graph_networks(args)
    ...
    def evaluate(rng_key: jnp.ndarray, params: optax.Params):
        ...
        def eval_one_batch(batch_idx, carry):
            ...
            def loop_fn(tup):
                ...
                rh_out = _reward_history_model.apply(
                    params,
                    state.observation["node_features"],  # type: ignore
                    state.observation["senders"],  # type: ignore
                    state.observation["receivers"],  # type: ignore
                ...
                # Zero out initial state reward history
                rh_out = rh_out.q_dist * (1 - (step == 0))
                ...
                greedy_action = jnp.where(was_terminated, -1, greedy_action)
            ...
            max_steps = 100  # Arbitrary cap to unbounded loops in case of bugs
            ...
            R_risks = jax.vmap(estimate_distortion_risk, in_axes=(0, 0, None, None))(
    ...
    def selfplay(
        ...
        Environments are auto-reset on termination, advancing to the next
        problem instance.
        ...
        def step_fn(
            ...
            # Zero out initial state reward history
            rh_out = rh_out.q_dist * (1 - (episode_stats["episode_length"] == 0))[:, None]
            ...
            samples = qr_samples + rh_samples[:, None, :]  # (batch_size, num_actions, num_samples)
    ...
    def learning_step(rng_key, params, target_params, opt_state, buffer_state):
        """Perform one gradient step jointly on the QR and reward-history heads.
        ...
        # Zero out initial state reward history
        rh_out = rh_out.q_dist * (1 - (learn_batch.second.step_count == 0))[:, None]

        rng_key, sample_key = jax.random.split(rng_key)

        # Join batch and action dims
        qr_dist = jnp.permute_dims(
            qr_next_out.q_dist, (0, 2, 1)
        )  # (batch_size, num_actions, num_quantiles)
        qr_dist = qr_dist.reshape(
            (-1, args.num_quantiles)
        )  # (batch_size * num_actions, num_quantiles)
        ...
        reward_history_prev = _reward_history_model.apply(
            target_params,
            learn_batch.first.obs["node_features"],
            learn_batch.first.obs["senders"],
            learn_batch.first.obs["receivers"],
            learn_batch.first.obs["aux"],
        ).q_dist  # (b, num_quantiles)
        reward_history_target = (
            learn_batch.first.reward[:, None]
            + (1 - learn_batch.first.initial[:, None]) * args.gamma * reward_history_prev
        )
        ...
        def _loss_fn(params: optax.Params) -> jnp.ndarray:
            ...
            # Reward History Loss
            reward_history_out = _reward_history_model.apply(
                params,
                learn_batch.second.obs["node_features"],
                learn_batch.second.obs["senders"],
                learn_batch.second.obs["receivers"],
    ...
    def eval_fn(subkey, iteration, params):
        ...
        jax.debug.print(
            "Iter {i} / {max_num_iters}, Eval Reward: {r}, Eval risk: {r_risk} ({r_opt_cvar}), Opt Expected: {r_opt_expected}",
            i=iteration,
...
if __name__ == "__main__":
    ...
    def save_logs(logs):
        ...
        """
        log_file_path = "./logs/sampled_tql/smis/logger.log"
