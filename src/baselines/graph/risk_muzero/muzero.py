# Risk MuZero (graph/node) training loop
#
# Implements the full graph-node Risk MuZero training pipeline:
...
#   ObsEncoder          — graph observation → node / aux embeddings
...
from src.baselines.graph.risk_muzero.networks import make_network_apply_fns
from src.baselines.graph.risk_muzero.util import (
    ExItTransition,
    get_train_targets,
...
def run_experiment(args):
    """Run the full graph-node Risk MuZero training loop.
    ...
    def root_fn(
        ...
        node_embeddings, aux_embedding = representation_apply.apply(
            params,
    ...
    def recurrent_fn(
        ...
        node_embeddings, reward_dist, aux_embedding = recurrent_inference.apply(
            params,
            node_embeddings,
            aux_embedding,
            senders,
    ...
    def learning_step(params, target_params, opt_state, buffer_state, key):
        ...
        def loss_fn(
            ...
            root_node_reps, root_aux_embedding = representation_apply.apply(
                params,
            ...
            def unroll_fn(carry, targets):
                ...
                next_node_proj, next_aux_proj = projection_apply.apply(
                    params,
                    next_node_embeddings,
                    next_aux_embedding,
                )  # (b, num_hidden)
                ...
                sc_loss = sc_node_loss + sc_aux_loss
    ...
    def train_loop_body(
        ...
        jax.debug.print(
            ...
            "Value Loss: {value_loss:.4f}, Self-consistency Loss: {self_consistency_loss:.4f} (node: {node_self_consistency_loss:.4f}, aux: {aux_self_consistency_loss:.4f}), R: {reward_loss:.4f}, RH: {reward_history_loss:.4f}",
            i=iteration,
            max_num_iters=args.max_num_iters,
            loss=jnp.mean(loss),
...
if __name__ == "__main__":
    ...
    def save_logs(logs):
        ...
        """
        log_file_path = "./logs/rmz/smis/logger.log"
