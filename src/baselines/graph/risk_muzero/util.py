# Risk MuZero (graph/node) utilities
#
# Shared helpers used by the graph-node Risk MuZero training loop.
#
# Contents
# --------
...
def init_model_and_optim(env, model_init_fn, args):
    ...
    Constructs dummy inputs from a reset observation and calls the joint
    ``model_init_fn`` in a single forward pass.
...
def make_buffer(
    ...
    Stores full ``ExItTransition`` trajectories for MuZero-style unrolled
    training.  All buffer operations are JIT-compiled.
