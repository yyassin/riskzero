...
def instantiate_tree_from_root(
    ...
    root: base.RiskRootFnOutput,
    num_simulations: int,
    root_invalid_actions: chex.Array,
    extra_data: Any,
    utility_fn: base.SampleBasedUtilityFn,
    ...
    chex.assert_rank(root.value, 2)  # (B, num_quantiles)
    chex.assert_equal_shape([root.value, root.return_history])  # (B, num_quantiles)
    ...
    def _zeros_action(x):
        return jnp.zeros(batch_node_action + x.shape[1:], dtype=x.dtype)
    ...
    tree = Tree(
        ...
        # children_rewards=jax.tree.map(_zeros_action, root.return_history),  # type: ignore
        children_rewards=jax.tree.map(_zeros_action, root.return_history),  # type: ignore
        children_discounts=jnp.ones(batch_node_action, dtype=data_dtype),  # type: ignore
        embeddings=jax.tree.map(_zeros_node, root.embedding),  # type: ignore
        root_invalid_actions=root_invalid_actions,  # type: ignore
        extra_data=extra_data,  # type: ignore
        unbatched_q_value_fn=tree_lib.unbatched_trajectory_qvalues,
    )

    # And fill the root node.
    root_index = jnp.full([batch_size], Tree.ROOT_INDEX)

    # The root value is the return history plus the future value
    rng_key, subkey = jax.random.split(rng_key)
    sample_key = jax.random.split(subkey, batch_size)  # (B,)
    value_samples = jax.vmap(util.sample_quantile_distribution, in_axes=(0, 0, None))(
        sample_key, root.value, num_quantile_samples
    )  # (B, NUM_SAMPLES)
    rng_key, subkey = jax.random.split(rng_key)
    sample_key = jax.random.split(subkey, batch_size)  # (B,)
    return_history_samples = jax.vmap(util.sample_quantile_distribution, in_axes=(0, 0, None))(
        sample_key, root.return_history, num_quantile_samples
    )  # (B, NUM_SAMPLES)
    trajectory_samples = return_history_samples + value_samples  # type: ignore
    trajectory_value = jax.vmap(utility_fn)(trajectory_samples)  # type: ignore

    tree = update_tree_node(tree, root_index, root.prior_logits, trajectory_value, root.embedding)
    return tree
...
def risk_search(
    ...
    *,
    ...
    Args:
        ...
        utility_fn: a function that computes the utility of a trajectory return.
        num_quantile_samples: number of quantile samples to use for the utility.
        discount_factor: the discount factor to use for the search.
        sample_history: whether to sample the return history during simulation.
    ...
    invalid_actions = (
        invalid_actions if invalid_actions is not None else jnp.zeros_like(root.prior_logits)
    )
    ...
    def body_fun(sim, loop_state):
        ...
        step_count = root.embedding[-1]  # (b,)
        parent_index, action, return_history, discount = simulate(
            simulate_keys,
            tree,
            action_selection_fn,
            max_depth,
    ...
    rng_key, instantiation_key = jax.random.split(rng_key)
...
class _SimulationState(NamedTuple):
    ...
    depth: int
    is_continuing: bool
    return_history: chex.Array
    discount: chex.Array


# simulation is vmapped over keys and trees
@functools.partial(jax.vmap, in_axes=[0, 0, None, None, 0, 0, None, None, None], out_axes=0)
def simulate(
    rng_key: chex.PRNGKey,
    tree: Tree,
    ...
    Args:
        ...
        return_history: the return history of the root node, used to compute the
            return of the simulation. Shape `[B, num_quantiles]`.
        step_count: the step count of the root node, used to compute the discount.
        num_quantile_samples: number of quantile samples to use for the utility.
        discount_factor: the discount factor to use for the search.
    ...
    def body_fun(state: _SimulationState) -> _SimulationState:
        ...
        reward_quantiles = tree.children_rewards[node_index, action]  # type: ignore
        reward_samples = util.sample_quantile_distribution(
            sample_key, reward_quantiles, num_quantile_samples
        )  # (NUM_SAMPLES,)
        return_history = state.return_history + state.discount * reward_samples
        ...
        return _SimulationState(
            ...
            return_history=return_history,  # type: ignore
            # Unvisited actions have a discount of 1.0 so this becomes a no-op.
            discount=state.discount * tree.children_discounts[node_index, action],  # type: ignore
        )
    ...
    rng_key, sample_key = jax.random.split(rng_key)
    return_history_samples = jax.lax.cond(
        sample_history,
        lambda: util.sample_quantile_distribution(
            sample_key, return_history, num_quantile_samples
        ),  # (NUM_SAMPLES,)
        lambda: jnp.zeros((num_quantile_samples)),  # type: ignore
    )

    init_discount = discount_factor**step_count
    initial_state = _SimulationState(
        ...
        return_history=return_history_samples,
    ...
    return (
        end_state.node_index,
        end_state.action,
        end_state.return_history,
        end_state.discount,
    )  # type: ignore
...
def expand(
    ...
    Args:
        ...
        return_history: the return history of the root node, used to compute the
            return of the expansion. Shape `[B, num_quantiles]`.
        discount: the discount factor for the expansion. Shape `[B]`.
        utility_fn: a function that computes the utility of a trajectory return.
        num_quantile_samples: number of quantile samples to use for the utility.
    ...
    """
    batch_size = tree_lib.infer_batch_size(tree)
    num_quantiles = tree.children_rewards.shape[-1]
    batch_range = jnp.arange(batch_size)
    ...
    chex.assert_shape(step.reward, [batch_size, num_quantiles])
    chex.assert_shape(step.discount, [batch_size])
    chex.assert_shape(step.value, [batch_size, num_quantiles])

    rng_key, subkey = jax.random.split(rng_key)
    sample_keys = jax.random.split(subkey, batch_size)  # (B,)
    value_samples = jax.vmap(util.sample_quantile_distribution, in_axes=(0, 0, None))(
        sample_keys, step.value, num_quantile_samples
    )  # (B, NUM_SAMPLES)

    rng_key, subkey = jax.random.split(rng_key)
    sample_keys = jax.random.split(subkey, batch_size)  # (B,)
    reward_samples = jax.vmap(util.sample_quantile_distribution, in_axes=(0, 0, None))(
        sample_keys, step.reward, num_quantile_samples
    )  # (B, NUM_SAMPLES)

    trajectory_return = (
        return_history
        + discount[:, None] * (reward_samples + step.discount[:, None] * value_samples)  # type: ignore
    )
    trajectory_value = jax.vmap(utility_fn)(trajectory_return)  # type: ignore
    chex.assert_shape(trajectory_value, (batch_size,))

    # Update the expanded node.
    tree = update_tree_node(tree, next_node_index, step.prior_logits, trajectory_value, embedding)
...
def backward(tree: Tree[T], leaf_index: chex.Numeric) -> Tree[T]:
    ...
    def body_fun(loop_state):
        ...
        # Update parent value. Rewards and discounts have been account for during simulation.
        bp_value = leaf_value
        parent_value = (tree.node_values[parent_index] * count + bp_value) / (count + 1.0)
        ...
        tree = tree.replace(
            ...
            children_values=update(tree.children_values, children_values, parent_index, action),
            # Update visits of the children of this parent node.
            children_visits=update(tree.children_visits, children_counts, parent_index, action),
        )
