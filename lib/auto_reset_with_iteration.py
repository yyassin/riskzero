# auto_reset_with_iteration
#
# A variant of pgx's auto_reset wrapper that increments state.iteration on
# each episode reset. This allows the environment to cycle through a dataset
# of problem instances — each new episode advances to the next instance rather
# than always reinitializing with the same seed.
    ...
    def wrapped_step_fn(state: State, action: Array, key: Optional[PRNGKey] = None):
        ...
        state = jax.lax.cond(
            ...
            lambda: state.replace(  # type: ignore
                ...
                _step_count=jnp.zeros_like(state._step_count),  # type: ignore
            ),
            lambda: state,
        )
        ...
        state = jax.lax.cond(
            ...
            lambda: init_fn(
                ...
                state.offset,  # type: ignore
                state.num_envs,  # type: ignore
                state.split,  # type: ignore
            ).replace(
